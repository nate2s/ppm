...
// and free routines.
#define dcMemory_allocate(a) decDcMemory_Malloc(a)
#define dcMemory_free(a) decFree(a)
#define DECFENCE 0x5a              // corruption detector
// 'Our' malloc and free:
static void *decDcMemory_Malloc(size_t);
...
decNumber * decNumberFromString(decNumber *dn, const char chars[],
  ...
  do {                             // status & malloc protection
    ...
    if (d<=set->digits) res=dn->lsu;    // fits into supplied decNumber
      ...
      if (needbytes>(Int)sizeof(resbuff)) { // too big for local
        allocres=(Unit *)dcMemory_allocate(needbytes);
  ...
  if (allocres!=NULL) dcMemory_free(allocres);   // drop any storage used
...
decNumber * decNumberAbs(decNumber *res, const decNumber *rhs,
                         decContext *set) {
    decNumber *dzero = dcDecNumber_createWithLsuSize(set->digits); // for 0
...
decNumber * decNumberCompareTotalMag(decNumber *res, const decNumber *lhs,
  ...
  decNumber *bufa = dcDecNumber_createWithLsuSize(DECBUFFER + 1);
  decNumber *bufb = dcDecNumber_createWithLsuSize(DECBUFFER + 1);
  ...
  do {                                  // protect allocated storage
    ...
    if (decNumberIsNegative(lhs)) {     // lhs<0
      ...
      dcDecNumber_resize(bufa, needbytes, &status);
    ...
    if (decNumberIsNegative(rhs)) {     // rhs<0
      ...
      dcDecNumber_resize(bufb, needbytes, &status);
  ...
  if (allocbufa!=NULL) dcMemory_free(allocbufa); // drop any storage used
  if (allocbufb!=NULL) dcMemory_free(allocbufb); // ..
  if (status!=0) decStatus(res, status, set);

  dcDecNumber_free(&bufa);
  dcDecNumber_free(&bufb);
...
decNumber * decNumberExp(decNumber *res, const decNumber *rhs,
  ...
  #if DECSUBSET
  if (allocrhs !=NULL) dcMemory_free(allocrhs);  // drop any storage used
  #endif
...
decNumber * decNumberFMA(decNumber *res, const decNumber *lhs,
  ...
  uInt needbytes;                  // for space calculations
  decNumber *bufa = dcDecNumber_createWithLsuSize(DECBUFFER * 2 + 1);
  decNumber *allocbufa=NULL;       // -> allocated bufa, iff allocated
  decNumber *acc;                  // accumulator pointer
  decNumber *dzero = dcDecNumber_createWithLsuSize(set->digits);       // work
...
decNumber * decNumberLogB(decNumber *res, const decNumber *rhs,
  ...
  if (decNumberIsNaN(rhs)) decNaNs(res, rhs, NULL, set, &status);
    ...
    {
        // temporary number
        decNumber *buft = dcDecNumber_createWithLsuSize(10);
        decNumberFromInt32(buft, ae);        // lay it out
        decNumberPlus(res, buft, set);       // round as necessary
        dcDecNumber_free(&buft);
    }
   }
...
decNumber * decNumberLog10(decNumber *res, const decNumber *rhs,
  ...
  decNumber *bufa = dcDecNumber_createWithLsuSize(DECBUFFER + 2);
  decNumber *bufb = dcDecNumber_createWithLsuSize(DECBUFFER + 2);
  decNumber *bufw = dcDecNumber_createWithLsuSize(10);
  ...
  if (allocbufb!=NULL) dcMemory_free(allocbufb); // ..
  #if DECSUBSET
  if (allocrhs !=NULL) dcMemory_free(allocrhs);  // ..
  #endif
  // apply significant status
  if (status!=0) decStatus(res, status, set);
  #if DECCHECK
...
decNumber * decNumberMinus(decNumber *res, const decNumber *rhs,
                           decContext *set) {
    decNumber *dzero = dcDecNumber_createWithLsuSize(set->digits);
  uInt status=0;                        // accumulator
...
decNumber * decNumberNextMinus(decNumber *res, const decNumber *rhs,
                               decContext *set) {
    decNumber *dtiny = dcDecNumber_createWithLsuSize(set->digits); // constant
...
decNumber * decNumberNextPlus(decNumber *res, const decNumber *rhs,
                              decContext *set) {
  // constant
  decNumber *dtiny = dcDecNumber_createWithLsuSize(set->digits);
  decContext workset=*set;                   // work
  uInt status=0;                             // accumulator
  #if DECCHECK
  if (decCheckOperands(res, DECUNUSED, rhs, set)) return res;
...
decNumber * decNumberPower(decNumber *res, const decNumber *lhs,
  ...
  decContext aset;                 // working context
  // work value 1...
  decNumber *dnOne = dcDecNumber_createWithLsuSize(set->digits);
  // local accumulator buffer [a decNumber, with digits+elength+1 digits]
  decNumber *dacbuff = dcDecNumber_createWithLsuSize(DECBUFFER + 9);
  decNumber *invbuff = dcDecNumber_createWithLsuSize(DECBUFFER + 9);
  ...
  do {                             // protect allocated storage
    ...
    dcDecNumber_resize(dacbuff, needbytes, &status);
      ...
      *dac->lsu=1;                      // ..
      ...
      if (decNumberIsNegative(rhs)) {   // was a **-n [hence digits>0]
        ...
        if (set->extended) {            // need to calculate 1/lhs
          ...
          dcDecNumber_resize(invbuff, needbytes, &status);
  ...
  if (allocdac!=NULL) dcMemory_free(allocdac);   // drop any storage used
  if (allocinv!=NULL) dcMemory_free(allocinv);   // ..
  #if DECSUBSET
  if (alloclhs!=NULL) dcMemory_free(alloclhs);   // ..
  if (allocrhs!=NULL) dcMemory_free(allocrhs);   // ..
  #endif
  if (status!=0) decStatus(res, status, set);
  #if DECCHECK
  ...
  dcDecNumber_free(&invbuff);
  dcDecNumber_free(&dacbuff);
  dcDecNumber_free(&dnOne);
    ...
    if (reqexp==BADINT                       // something bad ..
     ...
     || (abs(reqexp)+1)/2>(set->digits+set->emax)) // .. or out of range
      ...
      if (!decNumberIsInfinite(res)) {       // prepare to scale
        Int myExp=res->exponent;               // save for overflow test
        res->exponent+=reqexp;               // adjust the exponent
        if (((myExp^reqexp)>=0)                // same sign ...
         && ((myExp^res->exponent)<0)) {       // .. but result had different
          // the calculation overflowed, so force right treatment
          if (myExp<0) res->exponent=DEC_MIN_EMIN-DEC_MAX_DIGITS;
...
decNumber * decNumberSquareRoot(decNumber *res, const decNumber *rhs,
  ...
  decNumber *dzero = dcDecNumber_create(); // used for constant zero
  Int  maxp;                       // largest working precision
  Int  workp;                      // working precision
  Int  residue=0;                  // rounding residue
  uInt status=0, ignore=0;         // status accumulators
  uInt rstatus;                    // ..
  Int  myExp;                        // working exponent
  ...
  decNumber *buff = dcDecNumber_createWithLsuSize(DECBUFFER + 1);
  ...
  decNumber *buft = dcDecNumber_createWithLsuSize(3);
  ...
  do {                             // protect allocated storage
    ...
    if (! dcDecNumber_resize(buff, needbytes, &status))
    {
        break;
    }
    ...
    if (! dcDecNumber_resize(bufa, needbytes, &status))
    {
        break;
    }

    if (! dcDecNumber_resize(bufb, needbytes, &status))
    {
        break;
    }
    ...
    decNumberCopy(f, rhs);
    myExp=f->exponent+f->digits;               // adjusted to Hull rules
    ...
    a->exponent+=myExp/2;                      // set correct exponent
    ...
    a->exponent-=myExp/2;                      // back to 0.1->1
    ...
    if (decNumberIsNegative(b)) {                 // f < b [i.e., b > f]
      ...
      approxset.emin-=myExp/2;                      // adjust to match a
      approxset.emax-=myExp/2;
  ...
  if (allocbuff!=NULL) dcMemory_free(allocbuff);      // drop any storage used
  if (allocbufa!=NULL) dcMemory_free(allocbufa);      // ..
  if (allocbufb!=NULL) dcMemory_free(allocbufb);      // ..
  #if DECSUBSET
  if (allocrhs !=NULL) dcMemory_free(allocrhs);       // ..
  #endif
...
decNumber * decNumberToIntegralExact(decNumber *res, const decNumber *rhs,
                                     decContext *set) {
    decNumber *dn = dcDecNumber_createWithLsuSize(set->digits);
...
enum decClass decNumberClass(const decNumber *dn, decContext *set) {
  ...
  if (decNumberIsNegative(dn)) return DEC_CLASS_NEG_SUBfalseRMAL;
  return DEC_CLASS_POS_SUBfalseRMAL;
  } // decNumberClass
...
const char *decNumberClassToString(enum decClass eclass) {
  ...
  if (eclass==DEC_CLASS_POS_SUBfalseRMAL) return DEC_ClassString_PS;
  if (eclass==DEC_CLASS_NEG_SUBfalseRMAL) return DEC_ClassString_NS;
...
static void decToString(const decNumber *dn, char *string, Flag eng) {
  Int myExp=dn->exponent;       // local copy
  Int e;                      // E-part value
  ...
  if (dn->bits&DECSPECIAL) {       // Is a special value
    ...
    // NaN string
    if (myExp!=0 || (*dn->lsu==0 && dn->digits==1)) return;
  ...
  if (myExp==0) {                    // simple integer [common fastpath]
    for (;up>=dn->lsu; up--) {     // each Unit from msu
      u=*up;                       // contains DECDPUN digits to lay out
      for (; cut>=0; c++, cut--) TODIGIT(u, cut, c, myPow);
  ...
  pre=dn->digits+myExp;              // digits before '.'
  e=0;                             // no E
  if ((myExp>0) || (pre<-5)) {       // need exponential form
    e=myExp+dn->digits-1;            // calculate E value
...
static decNumber * decAddOp(decNumber *res, const decNumber *lhs,
    ...
    if ((maxdigits>=reqdigits)          // is, or could be, too large
     || (res==rhs && rhsshift>0)) {     // destructive overlap
      ...
      if (need*sizeof(Unit)>sizeof(accbuff)) {
        ...
        allocacc=(Unit *)dcMemory_allocate(need*sizeof(Unit));
  ...
  if (allocacc!=NULL) dcMemory_free(allocacc);        // drop any storage used
  #if DECSUBSET
  if (allocrhs!=NULL) dcMemory_free(allocrhs);        // ..
  if (alloclhs!=NULL) dcMemory_free(alloclhs);        // ..
  #endif
...
static decNumber * decDivideOp(decNumber *res,
  ...
  do {                             // protect allocated storage
    ...
    if (acclength*sizeof(Unit)>sizeof(accbuff)) {
      ...
      allocacc=(Unit *)dcMemory_allocate(acclength*sizeof(Unit));
      if (allocacc==NULL) {             // hopeless -- abandon
        *status|=DEC_Insufficient_storage;
        break;}
    ...
    if ((var1units+1)*sizeof(Unit)>sizeof(varbuff)) {
      ...
      varalloc=(Unit *)dcMemory_allocate((var1units+1)*sizeof(Unit));
    ...
    for (myPow=&powers[1]; *msu1>=*myPow; myPow++) exponent--;
    for (myPow=&powers[1]; *msu2>=*myPow; myPow++) exponent++;
      ...
      if (accunits!=0 || thisunit!=0) {      // is first or non-zero
        *accnext=thisunit;                   // store in accumulator
        ...
        if (accunits==0) {
          ...
          for (myPow=&powers[1]; thisunit>=*myPow; myPow++) accdigits++;
          }
    ...
    if (op&DIVIDE) {
      ...
      if (op & (REMAINDER|REMNEAR)) {
        ...
        if (*var1==0 && var1units==1) {      // residue is 0
          Int myExp=lhs->exponent;             // save min(exponents)
          if (rhs->exponent<myExp) myExp=rhs->exponent;
          decNumberZero(res);                // 0 coefficient
          #if DECSUBSET
          if (set->extended)
          #endif
          res->exponent=myExp;                 // .. with proper exponent
        ...
        if (op&REMNEAR) {
          ...
          if (compare>0 || (compare==0 && wasodd)) { // adjustment needed
            Int myExp, expunits, exprem;       // work
            ...
            myExp=rhs->exponent-exponent;      // RHS padding needed
            // Calculate units and remainder from exponent.
            expunits=myExp/DECDPUN;
            exprem=myExp%DECDPUN;
  ...
  if (varalloc!=NULL) dcMemory_free(varalloc);   // drop any storage used
  if (allocacc!=NULL) dcMemory_free(allocacc);   // ..
  #if DECSUBSET
  if (allocrhs!=NULL) dcMemory_free(allocrhs);   // ..
  if (alloclhs!=NULL) dcMemory_free(alloclhs);   // ..
  #endif
...
static decNumber * decMultiplyOp(decNumber *res, const decNumber *lhs,
  ...
  do {                             // protect allocated storage
    ...
    if (rhs->digits>NEEDTWO) {     // use fastpath...
      ...
      if (needbytes>(Int)sizeof(zlhibuff)) {
        alloclhi=(uInt *)dcMemory_allocate(needbytes);
        zlhi=alloclhi;}
      needbytes=irhs*sizeof(uInt);
      if (needbytes>(Int)sizeof(zrhibuff)) {
        allocrhi=(uInt *)dcMemory_allocate(needbytes);
        zrhi=allocrhi;}
      ...
      if (needbytes>(Int)sizeof(zaccbuff)) {
        allocacc=(uLong *)dcMemory_allocate(needbytes);
        zacc=(uLong *)allocacc;}
      ...
      if (needbytes>(Int)sizeof(accbuff)) {
        allocacc=(Unit *)dcMemory_allocate(needbytes);
  ...
  #endif
  #if FASTMUL
  if (allocrhi!=NULL) dcMemory_free(allocrhi);   // ..
  if (alloclhi!=NULL) dcMemory_free(alloclhi);   // ..
  #endif
  return res;
  } // decMultiplyOp
...
decNumber * decExpOp(decNumber *res, const decNumber *rhs,
  ...
  // DECBUFFER is 0
  decNumber *bufr = dcDecNumber_createWithLsuSize(DECBUFFER*2+1);
  ...
  decNumber *buft = dcDecNumber_createWithLsuSize(DECBUFFER*2+9+1);
  decNumber *allocbuft=NULL;       // -> allocated buft, iff allocated
  decNumber *t=buft;               // term
  // buffer for a, accumulator (working precision * 2), at least 9
  decNumber *bufa = dcDecNumber_createWithLsuSize(DECBUFFER*4+18+1);
  decNumber *allocbufa=NULL;       // -> allocated bufa, iff allocated
  decNumber *a=bufa;               // accumulator
  // decNumber for the divisor term; this needs at most 9 digits
  // and so can be fixed size [16 so can use standard context]
  decNumber *bufd = dcDecNumber_createWithLsuSize(16);
  decNumber *d=bufd;               // divisor
  decNumber numone = {0};               // constant 1
  numone.lsu = (uint8_t *)dcMemory_allocate(sizeof(decNumberUnit));
  numone.lsu[0] = 1;
  numone.digits = 1;
    ...
    if (h>8) {                          // overflow/underflow
      ...
      *a->lsu=2;                        // not 1 but < exp(1)
      ...
      if (rhs->exponent!=use) {
        ...
        if (! dcDecNumber_resize(bufr, needbytes, status))
        {
            break;
        }
      ...
      needbytes=sizeof(decNumber)+(D2U(p+2)-1)*sizeof(Unit);
      dcDecNumber_resize(buft, needbytes, status);
  ...
  if (allocrhs !=NULL) dcMemory_free(allocrhs);  // drop any storage used
  if (allocbufa!=NULL) dcMemory_free(allocbufa); // ..
  if (allocbuft!=NULL) dcMemory_free(allocbuft); // ..
  // [status is handled by caller]

  dcDecNumber_free(&bufr);
  dcDecNumber_free(&buft);
  dcDecNumber_free(&bufa);
  dcDecNumber_free(&bufd);
  dcMemory_free(numone.lsu);
...
decNumber * decLnOp(decNumber *res, const decNumber *rhs,
  ...
  // (adjustment calculator, same size)
  decNumber *bufa = dcDecNumber_createWithLsuSize(DECBUFFER + 12);
  decNumber *bufb = dcDecNumber_createWithLsuSize(DECBUFFER * 2 + 2);
  decNumber numone = {0};               // constant 1
  numone.lsu = (uint8_t *)dcMemory_allocate(sizeof(decNumberUnit));
  numone.lsu[0] = 1;
  numone.digits = 1;
  ...
  decNumber  cmp; // work
  memset(&cmp, 0, sizeof(decNumber));
  cmp.lsu = (uint8_t *)dcMemory_allocate(sizeof(decNumberUnit));
...
decNumber * decCompareOp(decNumber *res, const decNumber *lhs,
  ...
  #if DECSUBSET
  if (allocrhs!=NULL) dcMemory_free(allocrhs);   // free any storage used
  if (alloclhs!=NULL) dcMemory_free(alloclhs);   // ..
  #endif
  return res;
  } // decCompareOp
...
static Int decUnitCompare(const Unit *a, Int alength,
                          const Unit *b, Int blength, Int myExp) {
  Unit  *acc;                      // accumulator for result
  ...
  if (alength>blength+(Int)D2U(myExp)) return 1;
  if (alength+1<blength+(Int)D2U(myExp)) return -1;

  // Need to do a real subtract.  For this, a result buffer is needed
  // even though only the sign is of interest.  Its length needs
  // to be the larger of alength and padded blength, +2
  need=blength+D2U(myExp);                // maximum real length of B
  ...
  *dropped=0;                           // assume no zeros dropped
  ...
  for (d=0; d<dn->digits-1; d++) { // [don't strip the final digit]
    ...
    if (!all) {                    // trimming
      ...
      if (myExp<=0) {                // if digit might be significant
        if (myExp==0) break;         // then quit
        myExp++;                     // next digit might be significant
        }
      }
    cut++;                         // next power
...
static decNumber *decRoundOperand(const decNumber *dn, decContext *set,
  ...
  res = dcDecNumber_createWithLsuSize(set->digits);
  //res=(decNumber *)dcMemory_allocate(sizeof(decNumber)
...
static void decFinalize(decNumber *dn, decContext *set, Int *residue,
  ...
  if (dn->exponent<=tinyexp) {          // prefilter
    ...
    decNumber nmin = {0};
    nmin.lsu = (uint8_t *)dcMemory_allocate(sizeof(decNumberUnit));
    decNumberZero(&nmin);
    nmin.lsu[0]=1;
    nmin.exponent=set->emin;
    comp=decCompare(dn, &nmin, 1);                // (signless compare)
    dcMemory_free(nmin.lsu);
...
static Int decGetDigits(Unit *uar, Int len) {
  ...
  for (; up>=uar; up--) {
    ...
    for (myPow=&powers[4]; *up>=*myPow; myPow++) digits++;
    #endif
    #endif
    #endif
    #endif
...
/* ------------------------------------------------------------------ */
static void *decDcMemory_Malloc(size_t n) {
  uInt  size=n+12;                 // true size
  void  *alloc;                    // -> allocated storage
  uByte *b, *b0;                   // work
  uInt  uiwork;                    // for macros

  alloc=dcMemory_allocate(size);              // -> allocated storage
...
static void decFree(void *alloc) {
  ...
  for (b=b0+n+8; b<b0+n+12; b++) if (*b!=DECFENCE)
    printf("=== Corrupt byte [%02x] at offset +%d from %ld, n=%ld ===\n", *b,
           b-b0-8, (LI)b0, (LI)n);
  dcMemory_free(b0);                        // drop the storage
  decAllocBytes-=n;                // account for storage
  // printf(" free -- dAB: %d (%d)\n", decAllocBytes, -n);
  } // decFree
#define dcMemory_allocate(a) decDcMemory_Malloc(a)
